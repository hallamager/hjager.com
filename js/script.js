...
$(function() {
	...
	$(window).resize(function() {
		delay(function() {
				var gallery_images = $('#slide-8 img');
				...
				var gallery_width = $('#slide-8 img').width() * $('#slide-8 img').length;
				gallery_width /= 2;
				if ( $('#slide-8 img').length % 2 != 0 ) {
					gallery_width += $('#slide-8 img').width();
				}
				
				$('#slide-3 .row').css('width', gallery_width );
				
				var left_pos = $('#slide-8 .row').width() - $('body').width();
				left_pos /= -2;
...
jQuery(document).ready(function ($) {
	//Cache some variables
	var images = $('#slide-8 a');
	
	images.hover(
		function(e) {
			var asta = $(this).find('img');
			$('#slide-8 img').not( asta ).stop(false, false).animate(
				{
					opacity: .5
				},
				'fast',
		...
		function(e) {
			$('#slide-8 img').stop(false, false).animate(
				{
					opacity: 1
				},
				'fast',
	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-down':
				...
				break;
			case 'arrow-left':
				offset_left = $('#slide-8 .row').offset().left + 452;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
			case 'arrow-right':
				offset_left = $('#slide-8 .row').offset().left - 452;
				if ( offset_left < $('body').width() - $('#slide-8 .row').width() ) {
					offset_left = $('body').width() - $('#slide-8 .row').width();
				}
				break;
		}
		...
		if ( offset_left != false ) {
			if ( $('#slide-8 .row').width() != $('body').width() ) {
				$('#slide-8 .row').stop(false, false).animate({
					left: offset_left
				}, 1500, 'easeInOutQuart');
			}
		}
